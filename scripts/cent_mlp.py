...
class AudioMLP(nn.Module):
    def __init__(self, input_size_centroids, input_size_flux, hidden_size, num_classes):
        ...
        # Fully connected layers for spectral centroids
        self.fc_centroids = nn.Linear(input_size_centroids, hidden_size)
        
        # Fully connected layers for spectral flux
        self.fc_flux = nn.Linear(input_size_flux, hidden_size)
        
        # Concatenated fully connected layers
        self.fc1 = nn.Linear(hidden_size * 2, 64)  # Concatenating outputs of both MLPs
    ...
    def forward(self, x):
        ...
        x_flux = x[:,1,:]
        # Fully connected layers for spectral centroids
        h_n_centroids = torch.relu(self.fc_centroids(x_centroids))
        
        # Fully connected layers for spectral flux
        h_n_flux = torch.relu(self.fc_flux(x_flux))
        
        # Concatenate the outputs of both MLPs
        out = torch.cat((h_n_centroids, h_n_flux), dim=1)
...
input_size_centroids = input_size_flux = 2584
hidden_size = 64
num_classes = 4
        ...
        for learning_rate in lrs:
            ...
            wandb.init(
                ...
                config = {'batch_size': batch_size, 
                        ...
                        'epochs': epochs},
                name= "MLP2_"   + "_" + str(batch_size) + "_" + str(learning_rate)[2:] + "_" + str(hidden_size) + "_" + str(num_layers)
            ...
            model = AudioMLP(input_size_centroids, input_size_flux, hidden_size, num_classes=4)
